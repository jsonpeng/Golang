...
static void frm_snd_tmout_cb(void *data)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, sock_addr,
...
int frm_send(frm_layer_ctx_t *frm_ctx, frm_snd_prm_t *prm)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, prm->dst, zip_frm, frm_len,
                                (prm->wkup_poll)? TPT_SEND_FLG_USE_HOST_PORT : 0);
#else
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, prm->dst, zip_frm, frm_len, 0);
#endif
...
static int frm_keep_alive_resp_send(frm_layer_ctx_t *frm_ctx, struct sockaddr *dst_addr)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, dst_addr, zip_frm, ZIP_FRM_KEEP_ALIVE_SZ, 0);
...
static void    frm_tpt_rd_cb(struct _tpt_layer_ctx *tpt_ctx, uint8_t *buf,
                             uint16_t data_len, struct sockaddr *src_addr)
{
...
int frm_node_id_get(frm_layer_ctx_t *frm_ctx, struct sockaddr *zip_router, uint8_t *tgt_ip, int retry)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, zip_router, zip_frm, ZIP_FRM_ND_SOLICIT_SZ,
...
int frm_node_ipv6_get(frm_layer_ctx_t *frm_ctx, struct sockaddr *zip_router, uint8_t node_id)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, zip_router, zip_frm, ZIP_FRM_ND_INV_SOLICIT_SZ, 0);
...
int frm_gw_send(frm_layer_ctx_t *frm_ctx, uint8_t cls, uint8_t cmd, uint8_t *data, uint8_t data_len)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, gw_addr, frm,
                                (data)? (2 + data_len) : 2, 0);
