...
#define  MDNS_GET_ALL_ZWAVE_RESOURCES           ///< Get all z-wave resources in queries
#define  MDNS_GET_FIRST_ZWAVE_RESOURCE_ONLY     ///< Process the first z-wave resource in answers
...
typedef struct
{
    ...
    char                *name;          ///< null terminate utf-8 domain name, e.g.  Static Controller [5d9624d60100]._z-wave._udp.local
...
@return     Return 0 on success, negative error number on failure.
*/
static int mdns_encode_question(char *qname, uint16_t qname_offset, uint16_t qtype, uint8_t *buf, int *buf_len)
{
    int initial_buf_len;
    int res;
...
/**
mdns_parse_dname - Parse domain name
...
*/
static int mdns_parse_dname(uint8_t *dns_pkt, uint8_t *buf, int *buf_len, char **dname)
{
    int     initial_buf_len;
    int     bytes_processed;
    int     res;
    ...
    while (1)
    {
        ...
        {   //Compression
            ...
            res = mdns_parse_dname(dns_pkt, dns_pkt + offset, &comp_label_bytes, &comp_label);
            if (res != 0)
            {
                return -4;
            }
...
static int mdns_parse_rdata(uint16_t type, uint8_t *dns_pkt, uint8_t *buf, int *buf_len, void **rdata)
{
    ...
    switch (type)
    {
        case DNS_TYPE_PTR:
            {
                res = mdns_parse_dname(dns_pkt, buf, buf_len, (char **)rdata);
            }
            break;
        ...
        case DNS_TYPE_SRV:
            {
                ...
                *buf_len -= 6;
                ...
                res = mdns_parse_dname(dns_pkt, buf + 6, buf_len, &srv_rr.target_host);
...
static int mdns_parse_answer(uint8_t *dns_pkt, uint8_t *buf, int *buf_len, uint16_t ancnt, mdns_rr_t **resrc_rec)
{
    ...
    for (i=0; i<ancnt; i++)
    {
        ...
        res = mdns_parse_dname(dns_pkt, buf + bytes_processed, &remaining_bytes, &mdns_rr[i].name);
        if (res != 0)
        {
            debug_gw_discvr("mdns_parse_dname failed, err=%d\n", res);
            res = -3;
...
#ifdef  MDNS_GET_ALL_ZWAVE_RESOURCES
/**
mdns_host_name_find - find host name from resource records
...
*/
static mdns_host_rec_t *mdns_host_name_find(char *host_name, mdns_host_rec_t *zip_host_rec, int zip_host_rec_cnt)
{
    int i;

    for (i=0; i<zip_host_rec_cnt; i++)
    {
        if (zip_host_rec[i].host_name
            && (strcmp(host_name, zip_host_rec[i].host_name) == 0))
        {
...
/**
mdns_srv_name_find - find service name from resource records
@param[in]      srv_name        Service name
@param[in]      zip_host_rec    Z/IP resource records
@param[in]      zip_host_rec_cnt    Number of entries in zip_host_rec
@return pointer to the entry of Z/IP resource records if found; else return NULL
*/
static mdns_host_rec_t *mdns_srv_name_find(char *srv_name, mdns_host_rec_t *zip_host_rec, int zip_host_rec_cnt)
{
    int i;

    for (i=0; i<zip_host_rec_cnt; i++)
    {
        if (strcmp(srv_name, zip_host_rec[i].srv_name) == 0)
        {
            return &zip_host_rec[i];
        }
    }
...
/**
mdns_srv_name_save - save service name into resource records
@param[in]      srv_name        Service name
...
@return
*/
static void mdns_srv_name_save(char *srv_name, mdns_host_rec_t **zip_host_rec, int *zip_host_rec_cnt)
{
    ...
    {
        if (mdns_srv_name_find(srv_name, host_rec, host_rec_cnt))
        {   //Found, don't need to do anything
            return;
        }
    }
...
/**
mdns_zw_rsrc_get - get z-wave resource information
...
@return   Return 0 on success, negative error number on failure.
*/
static int mdns_zw_rsrc_get(int sock_fd, uint16_t ancnt, mdns_rr_t *rsrc_rec,
                            mdns_host_rec_t **zip_host_rec, int *zip_host_rec_cnt, int use_ipv4)
{
    ssize_t             bytes_sent;
    int                 res;
    int                 i;
    ...
    for (i=0; i<ancnt; i++)
    {
        if ((rsrc_rec[i].type == DNS_TYPE_PTR)
            && (strcmp((const char *)rsrc_rec[i].name, ZIP_RESOURCE_DOMAIN) == 0)
            && rsrc_rec[i].rr_data)
        {
            if (mdns_srv_name_find(rsrc_rec[i].rr_data, *zip_host_rec, *zip_host_rec_cnt))
            {   //Found, don't need to do anything
#ifdef  MDNS_GET_FIRST_ZWAVE_RESOURCE_ONLY
                break;
#else
                continue;
#endif
            ...
            res = mdns_encode_question(rsrc_rec[i].rr_data, 0, DNS_TYPE_SRV,
                                       mdns_buf + dname_offset, &free_buf_sz);
            if (res != 0)
            {
                return res;
            }
            //Save ther service name for verification of answer
            mdns_srv_name_save(rsrc_rec[i].rr_data, zip_host_rec, zip_host_rec_cnt);
            ...
            //
            //Get Z-wave resource info
            //
            free_buf_sz = 512 - DNS_PACKET_HEADER_SIZE - bytes_processed;

            res = mdns_encode_question(rsrc_rec[i].rr_data, dname_offset, DNS_TYPE_TXT,
...
/**
mdns_zw_rsrc_ip_get - get z-wave resource ip address
@param[in]      sock_fd         Socket file descriptor for sending DNS query
@param[in]      ancnt           Number of answers in resource records
@param[in]      rsrc_rec        Resource records
@param[out]     zip_host_rec    Buffer to store the service name selected for query
...
@return   Return 0 on success, negative error number on failure.
*/
static int mdns_zw_rsrc_ip_get(int sock_fd, uint16_t ancnt, mdns_rr_t *rsrc_rec,
                            mdns_host_rec_t **zip_host_rec, int *zip_host_rec_cnt, int use_ipv4)
{
    ssize_t             bytes_sent;
    int                 res;
    ...
    for (i=0; i<ancnt; i++)
    {
        if ((rsrc_rec[i].type == DNS_TYPE_SRV)
            && ((tmp_host_rec = mdns_srv_name_find(rsrc_rec[i].name, *zip_host_rec, *zip_host_rec_cnt))!= NULL)
            && (!tmp_host_rec->host_name)
            && rsrc_rec[i].rr_data)
        {   //
            //Get server IP
            //
            ...
            res = mdns_encode_question(srv_rr->target_host, 0, (use_ipv4)? DNS_TYPE_A : DNS_TYPE_AAAA,
                                       mdns_buf + free_buf_offset, &free_buf_sz);
            if (res != 0)
            {
                return res;
            }
            //Save ther host name for verification of answer
            tmp_host_rec->host_name = strdup(srv_rr->target_host);
            ...
            if (FD_ISSET(sfd, &readfds))
            {
                ...
                if (rcx_size > 0)
                {
                    ...
                    if (use_ipv4)
                    {
                        ...
                        debug_gw_discvr("Received from host IP:%s, port:%u \n", inet_ntoa(sa4.sin_addr), htons(sa4.sin_port));
                    }
                    else
                    {
...
//                              }
                                ...
                                if ((rsrc_rec->type == DNS_TYPE_PTR)
                                    && (strcmp((const char *)rsrc_rec->name, ZIP_RESOURCE_DOMAIN) == 0))
                                {
#ifdef  MDNS_GET_ALL_ZWAVE_RESOURCES
                                    //Get Z-wave resource info
                                    res = mdns_zw_rsrc_get(sfd, ancnt, rsrc_rec, &zip_host_rec, &zip_host_rec_cnt, use_ipv4);
                                    if (res != 0)
                                    {
                                        debug_gw_discvr("mdns_zw_rsrc_get with err:%d\n", res);
                                    }
#else
                                    if (use_ipv4)
                                    {
                                    ...
                                    gw_name[idx++] = strdup((char *)rsrc_rec->rr_data);

#endif
                                }

#ifdef  MDNS_GET_ALL_ZWAVE_RESOURCES
                                if (rsrc_rec->type == DNS_TYPE_SRV)
                                {
                                    tmp_host_rec = mdns_srv_name_find(rsrc_rec->name, zip_host_rec, zip_host_rec_cnt);

                                    if (tmp_host_rec)
                                    {
                                        if (!tmp_host_rec->host_name)
                                        {   //Get host IP
                                            res = mdns_zw_rsrc_ip_get(sfd, ancnt, rsrc_rec, &zip_host_rec, &zip_host_rec_cnt, use_ipv4);
                                            if (res != 0)
                                            {
                                                debug_gw_discvr("mdns_zw_rsrc_ip_get with err:%d\n", res);
                                            }
                                        }
                                        else
                                        {
                                            debug_gw_discvr("service:%s already has host:%s mapped\n",
                                                            tmp_host_rec->srv_name, tmp_host_rec->host_name);
                                        }
                                    }
                                    else
                                    {
                                        debug_gw_discvr("service %s is not z-wave resource\n", rsrc_rec->name);
                                    }
                                }
                                else if ((rsrc_rec->type == DNS_TYPE_A) && use_ipv4)
                                {
                                    tmp_host_rec = mdns_host_name_find(rsrc_rec->name, zip_host_rec, zip_host_rec_cnt);

                                    if (tmp_host_rec && rsrc_rec->rr_data)
                                    {
                                        memcpy(tmp_host_rec->host_ip, rsrc_rec->rr_data, IPV4_ADDR_LEN);
                                        debug_gw_discvr("service:%s, host:%s, IP:%u.%u.%u.%u\n",
                                                        tmp_host_rec->srv_name, tmp_host_rec->host_name,
                                                        tmp_host_rec->host_ip[0], tmp_host_rec->host_ip[1],
                                                        tmp_host_rec->host_ip[2], tmp_host_rec->host_ip[3]);

                                    }
                                    else
                                    {
                                        debug_gw_discvr("host %s is not z-wave resource\n", rsrc_rec->name);
                                    }
                                }
                                else if ((rsrc_rec->type == DNS_TYPE_AAAA) && !use_ipv4)
                                {
                                    tmp_host_rec = mdns_host_name_find(rsrc_rec->name, zip_host_rec, zip_host_rec_cnt);

                                    if (tmp_host_rec && rsrc_rec->rr_data)
                                    {
                                        memcpy(tmp_host_rec->host_ip, rsrc_rec->rr_data, IPV6_ADDR_LEN);
                                        debug_gw_discvr("service:%s, host:%s, IP:%02x%02x:%02x%02x: ...\n",
                                                        tmp_host_rec->srv_name, tmp_host_rec->host_name,
                                                        tmp_host_rec->host_ip[0], tmp_host_rec->host_ip[1],
                                                        tmp_host_rec->host_ip[2], tmp_host_rec->host_ip[3]);
