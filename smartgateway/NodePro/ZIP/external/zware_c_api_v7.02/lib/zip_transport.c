...
*/
int tpt_snd_req_create(tpt_layer_ctx_t *tpt_ctx, struct sockaddr *dst, uint8_t *buf, uint16_t dat_sz, uint16_t flag)
{
    ...
    if (tpt_ctx->use_ipv4)
    {
        send_pkt->dst.ipv4 = *((struct sockaddr_in *)dst);
    }
    else
    {
...
/**
tpt_wr_thrd - thread for sending IPv6 UDP packet
...
/**
tpt_rd_thrd - thread to receive IPv6 UDP packets
@param[in]	data		Context
@return
*/
#ifdef TCP_PORTAL
static void tpt_rd_thrd(void   *data)
{
        ...
        else
        {
            if (FD_ISSET(tpt_ctx->sock_fd, &readfds))
            {
                ...
                if (rcx_size > 0)
                {
                    ...
                    tpt_ctx->tpt_rd_cb(tpt_ctx, buf, rcx_size, (struct sockaddr *)&sa6);
    ...
    debug_msg_show(tpt_ctx->plt_ctx, "tpt_rd_thrd exited");
...
static void tpt_rd_thrd(void *data)
{
    ...
    while (tpt_ctx->rd_thrd_run)
    {
        ...
        else
        {
            if (FD_ISSET(tpt_ctx->sock_fd, &readfds))
            {
                ...
                rcx_size = recvfrom(tpt_ctx->sock_fd, buf, MAX_RCX_SIZE, 0,
                                    sock_addr, &sock_len);
                ...
                else if (rcx_size == SOCKET_ERROR)
                {
                    //Don't show error message if terminating thread
                    if (tpt_ctx->rd_thrd_run)
                    {
                        debug_msg_show(tpt_ctx->plt_ctx, "udp rcx error:%d", WSAGetLastError());
                    }
                    break;
                }
            }
...
#else
    //Setup polling
    poll_ctl.fd = tpt_ctx->sock_fd;
    poll_ctl.events = POLLIN;
    poll_ctl.revents = 0;

    //First receive with MSG_DONTWAIT flag, so it will not be blocked.
    //The first call to recvfrom() is necessary to enable the socket in
    //listening mode; else the call to poll() will never be successful.
    if (tpt_ctx->use_ipv4)
    {
        sock_len = sizeof(struct sockaddr_in);
        sock_addr = (struct sockaddr *)&sa4;
    ...
    rcx_size = recvfrom(tpt_ctx->sock_fd, buf, MAX_RCX_SIZE, MSG_DONTWAIT,
                        sock_addr, &sock_len);
    ...
    while (tpt_ctx->rd_thrd_run)
    {
        if (poll(&poll_ctl, 1, TRANSPORT_READ_TIMEOUT_MIN) > 0)
        {   //Check whether there is data to read
            if ((poll_ctl.revents & POLLIN) != 0)
            {
                if (tpt_ctx->use_ipv4)
                {
                    sock_len = sizeof(struct sockaddr_in);
                ...
                {
                    debug_msg_show(tpt_ctx->plt_ctx, "udp rcx error:%d", rcx_size);
                }
            }
        }
    }
...
static int tpt_thrd_start(tpt_layer_ctx_t *tpt_ctx)
{
    ...
    if (plt_cancel_thrd_create(tpt_wr_thrd, tpt_ctx, &tpt_ctx->wr_thrd_ctx) < 0)
    {
...
/**
tpt_setup - Configure network interface with IPv6 address and open socket
@param[in,out]	tpt_ctx		Context
@return     Return non-zero indicates success, zero indicates failure.
*/
...
#ifndef TCP_PORTAL
    // Configure network interface with IPv6 address and open socket
