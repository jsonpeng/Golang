...
static void tls_svr_thrd(void *data)
{
    ...
    fd_set              readfds;
    fd_set              writefds;   //For some of the fds encountered "ERROR_WANT_WRITE" during handshaking
    struct timeval      tv;
    int                 yes = 1;
    ...
    while (ctx->svr_thrd_run)
    {
        ...
        if (clnt_cnt > 0)
        {
            for (i=0; i<MAX_CLIENT_TLS; i++)
            {
                if (clnt_ctx[i].clnt_fd > 0)
                {
                    FD_SET(clnt_ctx[i].clnt_fd, &readfds);
                    ...
                    {
                        FD_SET(clnt_ctx[i].clnt_fd, &writefds);
                    }
                }
            }
        }
        ...
        fd_cnt = select(max_fdn + 1, &readfds, &writefds, NULL, &tv);

        if (fd_cnt == TPT_SOCKET_ERR)
        {
            //debug_gw_discvr("select failed with error %d\n", WSAGetLastError());
            debug_tls_svr("select failed\n");
        ...
        {
            if (FD_ISSET(ctx->sock_fd, &readfds))
            {
//              struct linger       so_linger;
                ...
                if ((new_fd = accept(ctx->sock_fd,(struct sockaddr *)&clnt_saddr, &clnt_addr_len)) < 0)
                {
                    ...
                    debug_tls_svr("tls_svr_thrd:accept with error:%d\n", new_fd);
                    continue;
                }
...
//                }
            ...
            if (fd_cnt > 0)
            {
                ...
                {
                    if ((clnt_ctx[i].clnt_fd > 0)
                        && (FD_ISSET(clnt_ctx[i].clnt_fd, &readfds) || FD_ISSET(clnt_ctx[i].clnt_fd, &writefds)))
                    {
                        result = tls_handshake_chk(ctx, &clnt_ctx[i]);
                        switch (result)
                        {
                            case TLS_HANDSHAKE_COMPLETED:
                                //Callback to hand over the fd and SSL object for secure read and write
                                if (!ctx->cb(clnt_ctx[i].clnt_fd, clnt_ctx[i].clnt_ssl, &clnt_ctx[i].clnt_prof, ctx->usr_param))
                            ...
                            case TLS_HANDSHAKE_AUTH_ERR:
                                //Set timeout
                                clnt_ctx[i].clnt_expire = ctx->tmr_tick + TLS_UNREGISTER_TIMEOUT;
                                break;
...
static int tls_svr_setup(tls_svr_ctx_t *ctx)
{
    ...
    //Enable server sesstion caching
    {
