...
#define ZIPV2_SESS_KEEP_ALIVE_CNT   3       ///< Z/IP version 2 secure session keep-alive count for SSL type client
...
#define DTLS_MAX_TIMEOUT_SEC        4       ///< DTLS maximum timeout in seconds. Note: timeout should start from 1; multiply by 2 in each iteration.
...
*/
static int dtls_func(tpt_layer_ctx_t *tpt_ctx, SSL *ssl, int func)
{
    int         curr_tmout = 1;
           ...
           (error == SSL_ERROR_WANT_READ || error == SSL_ERROR_WANT_WRITE))
    {
        ...
        if ((curr_tmout > DTLS_MAX_TIMEOUT_SEC) || (tpt_ctx->wr_thrd_run == 0))
        {
            break;
        }
...
//  }
    ...
    ret = SSL_shutdown(ssl_sock->ssl);
    if (ret == 0)
    {   //Wait for peer's "close notify" alert
        ret = SSL_shutdown(ssl_sock->ssl);
    }

    if (ret < 0)
    {
        int             error = SSL_get_error(ssl_sock->ssl, ret);
        dtls_select_e   select_ret;
        ...
        if ((error == SSL_ERROR_WANT_READ) || (error == SSL_ERROR_WANT_WRITE))
        {
            ...
            {
                select_ret = dtls_select(ssl_sock->sock_fd, DTLS_MAX_TIMEOUT_SEC);
                if ((select_ret == DTLS_SELECT_RECV_READY) || (select_ret == DTLS_SELECT_ERROR_READY))
                {
...
static int dtls_sock_create(tpt_layer_ctx_t *tpt_ctx, uint16_t port, int use_ipv4, int reuse_addr,
                            uint8_t *bound_addr, int *sock_fd)
{
    int res;
#ifdef OS_LINUX
    int old_flg;
...
@pre            Caller must lock dtls_ctx->rdwr_mtx
*/
static void dtls_rd_ready_hdlr(tpt_layer_ctx_t *tpt_ctx, fd_set *read_fds)
{
    int                 i;
    int                 k;
    int                 rd_err;
    ...
    for (i=0; i<MAX_ZWAVE_NODE; i++)
    {
        if (tbl[i].ipaddr_sz)
        {
            ...
            for (k=0; k<2; k++)
            {
                ...
                {
                    //Find read fd
                    if (FD_ISSET(ssl_sock->sock_fd, read_fds))
                    {
        #ifdef  DEBUG_ZIP_DTLS
                        ...
                        {   //Have valid data
                            ...
                            {   //Pass the message to upper layer
                                ...
                                plt_mtx_ulck(dtls_ctx->rdwr_mtx);
                                tpt_ctx->tpt_rd_cb(tpt_ctx, buf, rcx_size, sock_addr);
                                plt_mtx_lck(dtls_ctx->rdwr_mtx);
                            }
                        }
                        else if (rcx_size < 0)
...
l_PEEK_UDP_MSG:
    rcx_size = recvfrom(dtls_ctx->sock_fd, buf, MAX_RCX_SIZE, MSG_PEEK,
                        sock_addr, &sock_len);
    if (rcx_size > 0)
    {
        //Check if this message is "client hello"
        if (!(buf[0]==0x16 && buf[1]==0xfe && buf[2]==0xff && buf[13]==0x01))
        {   //Non "client hello" message, clear the read buffer
            rcx_size = recvfrom(dtls_ctx->sock_fd, buf, MAX_RCX_SIZE, 0, NULL, NULL);
        ...
        //Set listening destination port and address
        if (connect(dtls_ctx->sock_fd, (const struct sockaddr *)sock_addr, sock_len) != 0)
        {   //clear the read buffer
            recvfrom(dtls_ctx->sock_fd, buf, MAX_RCX_SIZE, 0, NULL, NULL);
            debug_msg_ts_show(tpt_ctx->plt_ctx, "New DTLS connect failed");
            return;
        }

#ifdef  DEBUG_ZIP_DTLS
        debug_msg_show(tpt_ctx->plt_ctx, "New DTLS connection request from:");
        ip_addr_show(tpt_ctx->plt_ctx, sock_addr, sock_len);
#endif
        ...
        SSL_set_fd(new_ssl, dtls_ctx->sock_fd);
        ...
        res = dtls_func(tpt_ctx, new_ssl, DTLS_SSL_ACCEPT);
        if (res != SSL_SUCCESS)
        {
            SSL_free(new_ssl);
            close(dtls_ctx->sock_fd);
            debug_msg_show(tpt_ctx->plt_ctx, "ssl_accept failed. Err: %d", res);
            goto l_CREATE_LISTENING_SOCKET;
        }

        if (tpt_ctx->use_ipv4)
        {
            dst_addr = (uint8_t *)&sa4.sin_addr.s_addr;
            dst_addr_len = IPV4_ADDR_LEN;
        }
        else
        {
            dst_addr = (uint8_t *)sa6.sin6_addr.s6_addr;
            dst_addr_len = IPV6_ADDR_LEN;
        }

        //Add to "IP address - ssl" table
        res = dtls_ip_ssl_add(dtls_ctx, new_ssl, dtls_ctx->sock_fd, dst_addr, dst_addr_len, DTLS_SSL_TYPE_SERVER, 0);
        if (res != 0)
        {
            SSL_free(new_ssl);
            close(dtls_ctx->sock_fd);
...
l_CREATE_LISTENING_SOCKET:
        //Create another listening socket to wait for new connection
        if (!dtls_sock_create(tpt_ctx, tpt_ctx->host_port, tpt_ctx->use_ipv4, 1,
...
void dtls_rd_thrd(tpt_layer_ctx_t *tpt_ctx)
{
    ...
    while (tpt_ctx->rd_thrd_run)
    {
        ...
        FD_SET(dtls_ctx->sock_fd, &readfds);//Server socket fd
        ...
        for (i=0; i<MAX_ZWAVE_NODE; i++)
        {
            if (tbl[i].ipaddr_sz)
            {
                ...
                for (k=0; k<2; k++)
                {
                    ...
                    if (ssl_sock->state == DTLS_SSL_STA_DATA || ssl_sock->state == DTLS_SSL_STA_SESS_EXPIRED)
                    {
                        FD_SET(ssl_sock->sock_fd, &readfds);
        ...
        fd_cnt = select(max_fdn + 1, &readfds, NULL, NULL, &tv);
        ...
        if (tpt_ctx->rd_thrd_run)
        {
            ...
            dtls_rd_ready_hdlr(tpt_ctx, &readfds);

            if (FD_ISSET(dtls_ctx->sock_fd, &readfds))
            ...
            while (1)
            {
                ...
                else
                {
                    ...
                    {   //using host listening port
                        res = dtls_sock_create(tpt_ctx, tpt_ctx->host_port, tpt_ctx->use_ipv4, 1,
                                               (tpt_ctx->use_ipv4)? NULL : tpt_ctx->listen_addr, &dst_fd);
                    }
                    else
                    {
                        res = dtls_sock_create(tpt_ctx, 0, tpt_ctx->use_ipv4, 0, NULL, &dst_fd);
                    }
                    ...
                    res = dtls_func(tpt_ctx, ssl, DTLS_SSL_CONNECT);
                    if (res != SSL_SUCCESS)
                    {
                        SSL_free(ssl);
                        close(dst_fd);
