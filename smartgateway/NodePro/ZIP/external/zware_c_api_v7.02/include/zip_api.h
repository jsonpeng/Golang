...
#ifndef WKUP_BY_MAILBOX_ACK ///<Not to use mailbox ACK of message as an indication of wakeup notification by sleeping node
    #define ENABLE_SEND_WKUP_NO_MORE_INFO   ///< Enable sending of wake up no more information
    #define DISABLE_MAILBOX	                ///< To send command to Z/IP gateway to disable mailbox feature
#endif
...
#define APPL_SUBVERSION     2   ///<application sub-version
...
typedef struct
{
    ...
    char            *dsk;       /**< Z/IP controller DSK string if ctl_cap has ZWNET_CTLR_CAP_S2 set */
}
zwnetd_t, *zwnetd_p;
...
#define IF_REC_TYPE_MAX         IF_REC_TYPE_ALRM_SNSR   /**< Maximun interface record type number */
/**
@}
*/
...
#define IF_REC_MAX_GROUPS       7   /**< Maximum number of groups for the controller to set its node id into the groups*/
...
typedef struct
{
    uint8_t     type;           /**< Binary sensor type ZW_BSENSOR_TYPE_XXX */
...
@{
*/
#define SEC_KEY_BITMSK_S2_K0     0x01    /**< S2: class key 0 (Ad-hoc devices)*/
#define SEC_KEY_BITMSK_S2_K1     0x02    /**< S2: class key 1 (Normal devices)*/
#define SEC_KEY_BITMSK_S2_K2     0x04    /**< S2: class key 2 (High Security devices)*/
#define SEC_KEY_BITMSK_S0        0x80    /**< S0: legacy security network key*/
/**
@}
*/
...
typedef struct
{
    ...
    union
    {
        uint8_t             req_key;    /**< For cb_type=S2_CB_TYPE_REQ_KEY; the joining device requested keys*/
        sec2_dsk_cb_prm_t   dsk;        /**< For cb_type=S2_CB_TYPE_DSK; the joining device DSK keys*/
    } cb_prm;

} sec2_add_cb_prm_t;

typedef void (*add_node_sec2_fn)(void *usr_param, sec2_add_cb_prm_t *cb_param);
/**<
...
typedef void (*get_dsk_fn)(zwnet_p net, char *dsk);
/**<
...
/**<
get Z/IP gateway DSK in security 2 mode
@param[in]	net	    network
...
#define ZWNET_NODE_STS_DOWN       1   ///< Node is either down or sleeping
...
typedef struct
{
	...
	uint8_t     KA_array_len;				/**< length/width of key attribute array*/
	uint8_t     KA_array[1];				/**< key attribute array place holder*/
} if_csc_data_t;
...
typedef void(*zwrep_csc_sup_fn)(zwifd_p ifd, uint8_t scene_cnt, uint8_t sameKA, uint8_t KA_array_len, uint8_t *KA_array, int valid);
/**<
