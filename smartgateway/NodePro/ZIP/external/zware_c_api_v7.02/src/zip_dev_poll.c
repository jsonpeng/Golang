...
static void zwdpoll_ep_sm_next(zwnet_p nw, uint8_t first_node_id)
{
    ...
    ep = (zwep_p)nw->ep_sm_job.first_ep;
    first_intf = ep->node->ep.intf;

    //Next endpoint becomes the first endpoint
    nw->ep_sm_job.first_ep = ep = (zwep_p)ep->obj.next;
    ...
    {   //This is a sleeping node
        ...
        //Callback poll sub-system
        zwpoll_on_wkup(intf->ep->node);

#ifdef ENABLE_SEND_WKUP_NO_MORE_INFO
        //Send WAKE_UP_NO_MORE_INFORMATION
        zwif_get_desc(intf, &ifd);
        zwif_exec_ex(&ifd, wakeup_resp, 2, zwif_tx_sts_cb, NULL, ZWIF_OPT_SKIP_MUL_Q, NULL);
#endif
    }
    ...
    if (zwnode_get_desc(first_intf->ep->node, &noded) == ZW_ERR_NONE)
    {
...
static int zwdpoll_node_updt_sm_start(zwnoded_p noded)
{
    ...
    sm_job.cb = zwdpoll_ep_sm_next;
    sm_job.first_ep = net->ep_sm_job.first_ep = &node->ep;
    sm_job.use_cache = 1;
