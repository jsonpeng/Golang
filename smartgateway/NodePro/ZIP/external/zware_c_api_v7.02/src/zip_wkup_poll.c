...
static void  wkuppoll_probe_cb(appl_layer_ctx_t *appl_ctx, int8_t tx_sts, void *user_prm, uint8_t node_id)
{
    ...
    if (tx_sts != TRANSMIT_COMPLETE_OK)
    {
        debug_wkup_ts_msg(appl_ctx->plt_ctx, "Wakeup poll node:%u, tx status:%d", node_id, tx_sts);
    }
...
static int wkuppoll_probe_updt_state(wupoll_ctx_t *wupoll_ctx,  wupoll_q_ent_t *wupoll_ent)
{
    ...
    result = zw_send_data(&wupoll_ctx->net->appl_ctx, &prm, wkuppoll_probe_cb, wupoll_ctx);

//  debug_wkup_ts_msg(&wupoll_ctx->net->plt_ctx, "wkuppoll_probe_updt_state: node id=%d, state=%u, res=%d",
//                     wupoll_ent->node_id, wupoll_ent->state, result);
    ...
    debug_wkup_ts_msg(&wupoll_ctx->net->plt_ctx, "wkuppoll_probe_updt_state: node id=%d, changed state=%u, res=%d",
                       wupoll_ent->node_id, wupoll_ent->state, result);
...
static void wkuppoll_tx_status_hdlr(wupoll_ctx_t *wupoll_ctx, wupoll_ev_msg_sts_t *msg)
{
    ...
    switch (msg->tx_sts)
    {
        ...
        case WUPOLL_WAKEUP_NOTIFICATION:
            debug_wkup_ts_msg(&wupoll_ctx->net->plt_ctx, "wkuppoll_tx_status_hdlr: tx_status:wkup_notif, node id=%d, state=%u",
                               wupoll_ent->node_id, wupoll_ent->state);
...
//  debug_wkup_ts_msg(&wupoll_ctx->net->plt_ctx, "wkuppoll_tx_status_hdlr: node id=%d, status=%u",
//                     wupoll_ent->node_id, wupoll_ent->state);
        ...
        if (util_tmr_exp_chk32(wupoll_ctx->tmr_tick, wupoll_ent->state_tmout))
        {
            switch (wupoll_ent->state)
            {
                ...
                case WUPOLL_STA_ACTIVE:
                    ...
                    if (wupoll_ent->state == WUPOLL_STA_ACTIVE)
                    {
...
*/
int wkuppoll_add(wupoll_ctx_t *wupoll_ctx, uint8_t node_id)
{
    wupoll_evt_msg_t msg;
