                  ...
                  int rpt_num, int total_rpt, char **gw_name)
{
    ...
    if (gw_addr_cnt > 0)
    {
        ...
        {   //Save gateway IP addresses
			...
			discvr_cb(gw_addr, gw_addr_buf_sz, hl_appl->gw_addr_cnt);
        }
    }
...
int gw_discovery()
{
    ...
    gw_discvr_ctx = zwnet_gw_discvr_start(gw_discvr_cb, &appl_ctx, appl_ctx.use_ipv4, 0 /*use mDNS for discovery*/);
