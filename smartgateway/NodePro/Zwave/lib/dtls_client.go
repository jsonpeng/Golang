...
static void set_proto_1_0(SSL_CTX *ctx) {
	SSL_CTX_set_min_proto_version(ctx, 0xFEFF); // 1.0
	SSL_CTX_set_max_proto_version(ctx, 0xFEFF); // 1.0
}
...
func NewDTLSContext() *DTLSCtx {
	...
	ctx := C.SSL_CTX_new(C.DTLSv1_client_method())
	if ctx == nil {
